...
public class AnswerDao extends AbstractDao<Answer, Long> {
    ...
    */
    ...
    private Query<Answer> survey_AnswersQuery;
    ...
    /** Internal query to resolve the "answers" to-many relationship of Survey. */
    public List<Answer> _querySurvey_Answers(long surveyId) {
        synchronized (this) {
            if (survey_AnswersQuery == null) {
                QueryBuilder<Answer> queryBuilder = queryBuilder();
                queryBuilder.where(Properties.SurveyId.eq(null));
                survey_AnswersQuery = queryBuilder.build();
            }
        }
        Query<Answer> query = survey_AnswersQuery.forCurrentThread();
    ...
    protected String getSelectDeep() {
        if (selectDeep == null) {
            ...
            builder.append(" LEFT JOIN SURVEY T0 ON T.'SURVEY_ID'=T0.'_id'");
            builder.append(' ');
